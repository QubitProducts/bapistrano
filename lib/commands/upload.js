...
module.exports = co(function * (options, bapConfig) {
  ...
  var releaseName = moment.utc().format(bapConfig.releaseNamePrefix) + '-' + commit.substr(0, 7)

  yield lifecycle.build(branch, releaseName, bapConfig)
  ...
  yield distFiles.reduce(function (m, f, i, l) {
    return m.then(write(f, i, l.length))
  }, Promise.resolve())
  spinner.stop()
  ...
  if (!isReleaseBranch) {
    ...
    yield lifecycle.notify(branch, releaseName, bapConfig)
  }
