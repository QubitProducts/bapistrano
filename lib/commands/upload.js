...
module.exports = function * (options) {
  // Build
  if (bapConfig.build) {
    log('Building with', color.blue(bapConfig.build))
    sh(bapConfig.build, true)
  }
  ...
  log('Uploading', color.blue(bapConfig.distDir), 'to', color.blue(path.join(branchDir, uploadDir)))
  ...
  if (bapConfig.clean) {
    log('Cleaning with', color.blue(bapConfig.clean))
