...
module.exports = async function list (selectedBranch, options, bapConfig) {
  ...
  var branches = await s3.readdir(bapConfig.uploadTo)
  ...
  for (var branch of branches) {
    ...
    var releases = await s3.readdir(path.join(bapConfig.uploadTo, branch, 'releases'))
    var numberOfReleases = releases.length
    releases = releases.sort().reverse().slice(0, options.limit)
    var current = await s3.readFile(path.join(bapConfig.uploadTo, branch, 'current'))
    ...
    for (var release of releases) {
      var uploadedAgo = moment.utc(release.substring(0, releaseIdPrefix.length), releaseIdPrefix).fromNow()
